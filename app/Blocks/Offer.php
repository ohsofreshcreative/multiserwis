...
class Offer extends Block
{
    ...
    public function fields()
    {
        ...
        $offer
            ...
            ->addSelect('selection_mode', [
                'label' => 'Tryb wyboru ofert',
                'choices' => [
                    'auto' => 'Automatyczny',
                    'manual' => 'Ręczny',
                ],
            ...
            ->addSelect('count', [
                ...
                ->conditional('selection_mode', '==', 'auto')
            ->addPostObject('selected_offers', [
                ...
                ->conditional('selection_mode', '==', 'manual')
            ->addText('section_id', [
                'label' => 'ID',
            ])
    ...
    {
        $selection_mode = get_field('selection_mode') ?: 'auto';
        $offers = [];
        $show_all_button = false;

        if ($selection_mode === 'manual') {
            $offers = get_field('selected_offers') ?: [];
        } else {
            $offers_count = get_field('count') ?: 'all';
            $limit = $offers_count === '4' ? 4 : -1;
            $offers = $this->getoffer($limit);
